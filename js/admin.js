...
(function () {
  ...
  function renderCustomers() {
    ...
    tbody.innerHTML = customers.length ? customers.map(c => `
      <tr>
        ...
        <td><div class="dtable-actions"><button class="icon-btn" data-open-customer="${c.id}" title="View"><i class="fas fa-eye"></i></button></div></td>
  ...
  function renderServices() {
    ...
    tbody.innerHTML = allServices.length ? allServices.map(s => `
      <tr>
        ...
        <td>${s.warranty_until ? window.fmtDate(s.warranty_until) : '—'}</td>
        <td></td>
  ...
  function bindCustomerForm() {
    ...
    $('#customerForm').addEventListener('submit', async (e) => {
      ...
      try {
        ...
        const { error: insertErr } = await window.sb.from('profiles').insert({
          id: newId, role: 'customer', full_name: name, phone, email
        });
  ...
  async function refreshCustomerDetail() {
    ...
    $('#detailVehiclesTable tbody').innerHTML = vehicles.length ? vehicles.map(v => `
      <tr><td><span class="svc-code">${v.code}</span></td><td>${window.escapeHtml(v.make)} ${window.escapeHtml(v.model||'')} ${v.year?('('+v.year+')'):''}</td><td>${window.escapeHtml(v.plate||'—')}</td></tr>
    `).join('') : `<tr><td colspan="3" style="color:var(--text-3);">No vehicles yet.</td></tr>`;

    $('#detailServicesTable tbody').innerHTML = services.length ? services.map(s => `
      <tr><td><span class="svc-code">${s.code}</span></td><td>${window.escapeHtml(s.service_type)}<br><span style="font-size:12px;color:var(--text-3);">${window.escapeHtml(s.package||'')}</span></td><td><span class="badge badge--${s.status}">${s.status.replace('_',' ')}</span></td><td></td></tr>
    `).join('') : `<tr><td colspan="4" style="color:var(--text-3);">No services yet.</td></tr>`;
  ...
  function bindServiceForm() {
    $('#addServiceBtn').addEventListener('click', () => openModal('modalService'));
    $('#serviceForm').addEventListener('submit', async (e) => {
      e.preventDefault();
      const months = Number($('#s_warranty').value) || 0;
      const warrantyUntil = months ? new Date(Date.now() + months * 30 * 86400000).toISOString().slice(0, 10) : null;
      const { error } = await window.sb.from('services').insert({
        customer_id: activeCustomerId,
