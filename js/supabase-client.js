...
(function () {
  ...
  function showFatalError(msg) {
    ...
    if (!loading) return; // page has no loading screen (e.g. login.html) — nothing to replace
    loading.innerHTML =
  ...
  // This script runs in <head>, before <body> (and #loading) exist yet, so
  // any UI we show has to wait for DOMContentLoaded. If the Supabase SDK is
  // already known to be missing, show the error right away; otherwise arm a
  // watchdog that catches any *other* reason the page might hang (dead
  // network, a stalled request, etc.) after a reasonable wait.
  ...
  if (!window.supabase) {
    ...
    window.logout = async function () { window.location.href = '/login.html'; };
    return;
  }
