/* ============================================================
JOHNSON SMART FILM — Main JavaScript v4.2
...
// Loader particles (desktop only & reduced motion check)
...
/* ═══════════════════════════════════════════════════════════
4. ACTIVE NAV LINK ON SCROLL (Throttled)
═══════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════
5. SCROLL REVEAL (Optimized Observer)
═══════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════
7. THREE.JS HERO SCENE (Desktop Only & Lazy Init)
═══════════════════════════════════════════════════════════ */
const canvasContainer = document.getElementById('heroCanvas');
// Only load the ~118KB three.js library at all if we're actually going to
// use it — desktop, motion allowed, and the canvas target exists. Mobile
// visitors (and reduced-motion users) never download it.
function initHeroScene() {
    ...
    try {
        ...
        const sphere = new THREE.Mesh(
            new THREE.SphereGeometry(1.4, 32, 32), // Reduced segments for performance
        ...
        const PARTICLE_COUNT = 400; // Reduced count
        ...
        // Pause when not visible to save battery/CPU
...
if (canvasContainer && !isMobile && !isLowPower) {
    const threeScript = document.createElement('script');
    threeScript.src = '/vendor/three/three.min.js';
    threeScript.onload = initHeroScene;
    threeScript.onerror = () => { canvasContainer.style.display = 'none'; };
...
if (hasFinePointer && !isMobile) {
    ...
    const interactiveElements = document.querySelectorAll('a, button, .pricing-card, .service-card');
...
if (isMobile || !hasFinePointer) {
    ...
    document.addEventListener('touchstart', e => {
        const now = Date.now();
        if (now - lastTap < 500) return; // Debounce
        lastTap = now;
...
/* ═══════════════════════════════════════════════════════════
10. PARALLAX BACKGROUND (Optimized)
═══════════════════════════════════════════════════════════ */
if (!isMobile) {
...
/* ═══════════════════════════════════════════════════════════
12. HERO TICKER — Seamless infinite loop
═══════════════════════════════════════════════════════════ */
const tickerTrack = document.querySelector('.hero__ticker-track');
if (tickerTrack && tickerTrack.children.length > 0) {
    const originalChildren = Array.from(tickerTrack.children);
    originalChildren.forEach(child => {
        tickerTrack.appendChild(child.cloneNode(true));
    });
