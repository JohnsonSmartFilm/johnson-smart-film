...
-- SECURITY: same privilege-escalation risk as the update trigger below,
-- but for INSERT — protects against it even if a public self-signup path
-- is added to the site later. A non-admin inserting their own row can
-- never set a role other than 'customer', no matter what the client sends.
create or replace function public.prevent_role_self_insert()
returns trigger language plpgsql
set search_path = public, pg_temp
as $$
begin
  if new.role is distinct from 'customer' and not public.is_admin() then
    new.role := 'customer';
  end if;
  return new;
end;
...
drop trigger if exists trg_prevent_role_self_insert on public.profiles;
create trigger trg_prevent_role_self_insert before insert on public.profiles
...
-- SECURITY: the USING clause above lets a customer update their OWN row,
-- which is needed so they can edit their name/phone from the dashboard.
-- But without a column-level restriction, that same permission would let
-- a customer send a raw API request that sets role = 'admin' on their own
-- profile — a full privilege-escalation to the admin panel. This trigger
-- closes that gap: any UPDATE on `role` that isn't performed by an existing
-- admin is silently reverted back to whatever the role already was, no
-- matter what the RLS policy above allows on the rest of the row.
